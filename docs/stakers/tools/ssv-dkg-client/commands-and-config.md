...
A special note goes to the `nonce` field, which represents how many validators the address identified in the `owner` parameter has already registered to the ssv.network.
You can keep track of this counter yourself, or you can use the SSV Subgraph tool. For more information, please refer to [the related user guide](/developers/api/subgraph-examples).
