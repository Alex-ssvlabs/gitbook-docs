...
Below is the educational context around Updating Operators, you can find actionable [steps in our separate guide](/stakers/cluster-management/update-operators.md).
...
1. Offboard from the network by removing the validator from its managing cluster.
2. Wait for at least two epochs to mitigate slashing risk associated with simultaneous validator setups.
3. Onboard the validator to the network by registering it to a new cluster.